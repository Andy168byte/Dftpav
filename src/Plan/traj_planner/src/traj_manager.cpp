...
ErrorType TrajPlanner::RunOnceParking() {
  ...
  if (!has_init_state_) {
    head_state_ = ego_vehicle_.state();
...
ErrorType TrajPlanner::RunMINCOParking() {
  ...
  if (flag_success) {
    ...
    for (unsigned int i = 0; i < kino_trajs_.size(); i++) {
      traj_container_.addSingulTraj((*ploy_traj_opt_->getMinJerkOptPtr())[i].getTraj(singul_container[i]),
                                    worldtime,
                                    ego_id_); // todo time
      std::cout << "init duration: " << duration_container[i] << std::endl;
      std::cout << "pieceNum: " << waypoints_container[i].cols() + 1 << std::endl;
      std::cout << "optimized total duration: "
                << (*ploy_traj_opt_->getMinJerkOptPtr())[i].getTraj(1).getTotalDuration() << std::endl;
...
void TrajPlanner::printLateralBehavior(LateralBehavior lateral_behavior) {
  ...
  std::cout << "\033[34m[LateralBehavior]The behavior is: " << lateral_behavior_str[int(lateral_behavior)] << "\033[0m"
            << std::endl;
}
...
ErrorType TrajPlanner::ConverSurroundTrajFromPoints(std::vector<std::vector<common::State>> sur_trajs,
...
ErrorType TrajPlanner::getRectangleConst(std::vector<Eigen::Vector3d> statelist) {
  ...
  for (const auto state : statelist) {
    ...
    while (NotFinishTable.norm() > 0) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        switch (i) {
          ...
          case 0:
            point1 = sourcePt
                + egoR * Eigen::Vector2d(sourceVp.length() / 2.0 + sourceVp.d_cr(), sourceVp.width() / 2.0);
            point2 =
                sourcePt + egoR * Eigen::Vector2d(-sourceVp.length() / 2.0 + sourceVp.d_cr(), sourceVp.width() / 2.0);
            newpoint1 = sourcePt
                + egoR * Eigen::Vector2d(sourceVp.length() / 2.0 + sourceVp.d_cr(), sourceVp.width() / 2.0 + step);
            newpoint2 = sourcePt
                + egoR * Eigen::Vector2d(-sourceVp.length() / 2.0 + sourceVp.d_cr(), sourceVp.width() / 2.0 + step);
          ...
          case 1:
            ...
            point2 =
                sourcePt + egoR * Eigen::Vector2d(sourceVp.length() / 2.0 + sourceVp.d_cr(), sourceVp.width() / 2.0);
            newpoint1 = sourcePt
                + egoR * Eigen::Vector2d(step + sourceVp.length() / 2.0 + sourceVp.d_cr(), -sourceVp.width() / 2.0);
            newpoint2 = sourcePt
                + egoR * Eigen::Vector2d(step + sourceVp.length() / 2.0 + sourceVp.d_cr(), sourceVp.width() / 2.0);
            //1 new1 new1 new2 new2 2
            map_itf_->CheckIfCollisionUsingLine(point1, newpoint1, &isocc, resolution / 2.0);
          ...
          case 3:
            ...
            point2 =
                sourcePt + egoR * Eigen::Vector2d(-sourceVp.length() / 2.0 + sourceVp.d_cr(), -sourceVp.width() / 2.0);
            newpoint1 = sourcePt
                + egoR * Eigen::Vector2d(-sourceVp.length() / 2.0 + sourceVp.d_cr() - step, sourceVp.width() / 2.0);
            newpoint2 = sourcePt
                + egoR * Eigen::Vector2d(-sourceVp.length() / 2.0 + sourceVp.d_cr() - step, -sourceVp.width() / 2.0);
            //1 new1 new1 new2 new2 2
            map_itf_->CheckIfCollisionUsingLine(point1, newpoint1, &isocc, resolution / 2.0);
    ...
    point1 = rawPt + egoR
        * Eigen::Vector2d(rawVp.length() / 2.0 + rawVp.d_cr() + expandLength[1], rawVp.width() / 2.0 + expandLength[0]);
    ...
    point2 = rawPt + egoR * Eigen::Vector2d(rawVp.length() / 2.0 + rawVp.d_cr() + expandLength[1],
    ...
    point3 = rawPt + egoR * Eigen::Vector2d(-rawVp.length() / 2.0 + rawVp.d_cr() - expandLength[3],
                                            -rawVp.width() / 2.0 - expandLength[2]);
    ...
    point4 = rawPt + egoR * Eigen::Vector2d(-rawVp.length() / 2.0 + rawVp.d_cr() - expandLength[3],
