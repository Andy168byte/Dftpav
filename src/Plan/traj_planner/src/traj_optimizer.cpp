...
double PolyTrajOptimizer::costFunctionCallback(void *func_data, const Eigen::VectorXd &x, Eigen::VectorXd &grad) {
  ...
  for (int trajid = 0; trajid < opt->trajnum; trajid++) {
    ...
    opt->jerkOpt_container[trajid].generate(P_container[trajid],
                                            T[trajid] / opt->piece_num_container[trajid],
                                            IniS,
                                            FinS);
  ...
  for (int trajid = 0; trajid < opt->trajnum; trajid++) {
    ...
    if (opt->GearOpt) {
      if (trajid > 0) {
        ...
        gradAngles[trajid - 1] +=
            gradIni.col(1).transpose() * Eigen::Vector2d(opt->non_sinv * sin(theta), -opt->non_sinv * cos(theta));
      }
      if (trajid < opt->trajnum - 1) {
        double theta = Angles[trajid];
        gradGear_container[trajid] += gradFin.col(0);
        gradAngles[trajid] +=
            gradFin.col(1).transpose() * Eigen::Vector2d(-opt->non_sinv * sin(theta), opt->non_sinv * cos(theta));
      }
    }
    ...
    opt->VirtualTGradCost(T[trajid],
                          t[trajid],
                          opt->jerkOpt_container[trajid].get_gdT() / opt->piece_num_container[trajid],
                          gradt[trajid],
                          time_cost);
...
int PolyTrajOptimizer::earlyExitCallback(void *func_data,
                                         const double *x,
                                         const double *g,
                                         const double fx,
                                         const double xnorm,
...
void PolyTrajOptimizer::VirtualTGradCost(const double &RT,
                                         const double &VT,
                                         const double &gdRT,
                                         double &gdVT,
                                         double &costT) {
...
void PolyTrajOptimizer::addPVAGradCost2CT(Eigen::VectorXd &costs, const int trajid, const double trajtime) {
  ...
  double step, alpha;
  Eigen::Matrix<double, 6, 2> gradViolaPc, gradViolaVc, gradViolaAc, gradViolaLatAc, gradViolaKc, gradViolaKLc,
      gradViolaKRc, gradViolaPhidotLc, gradViolaPhidotRc;
  double gradViolaPt, gradViolaVt, gradViolaAt, gradViolaLatAt, gradViolaKt, gradViolaKLt, gradViolaKRt,
      gradViolaPhidotLt, gradViolaPhidotRt;
  double violaPos, violaVel, violaAcc, violaLatAcc, violaCur, violaCurL, violaCurR, violaDynamicObs, violaPhidotL,
      violaPhidotR;
  double violaPosPenaD, violaVelPenaD, violaAccPenaD, violaLatAccPenaD, violaCurPenaD, violaCurPenaDL, violaCurPenaDR,
      violaDynamicObsPenaD, violaPhidotPenaDL, violaPhidotPenaDR;
  double violaPosPena, violaVelPena, violaAccPena, violaLatAccPena, violaCurPena, violaCurPenaL, violaCurPenaR,
      violaDynamicObsPena, violaPhidotPenaL, violaPhidotPenaR;
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (int j = 0; j <= K; ++j) {
          ...
          if (violaPos > 0) {
            ...
            gradViolaPc = beta0 * outerNormal.transpose() +
                beta1 * outerNormal.transpose()
                    * (singul_ * temp_l_Bl * z_h0 - ego_R * le * dsigma.transpose() * vel2_reci);
      ...
      if (violaCurL > 0.0) {
        ...
        //@hzc
        gradViolaKLc = beta1
            * (vel3_2_reci_e * ddsigma.transpose() * B_h - 3 * vel3_2_reci_e * vel2_reci_e * z_h3 * dsigma.transpose())
...
double PolyTrajOptimizer::debugGradCheck(const int i_dp, // index of constraint point
                                         ...
                                         const int sur_id,
                                         double res_t,
                                         Eigen::Matrix<double, 6, 2> c,
                                         int i,
                                         int j,
                                         double omg,
                                         double step,
                                         double wei_surround_,
                                         int K) {
  Eigen::Matrix2d ego_R, help_R;
  int singul_ = singul_container[trajid];
  ...
  Eigen::VectorXd signed_dists(8), ego_signed_dists1(4), ego_signed_dists2(4), ego_signed_dists3(4),
      ego_signed_dists4(4),
  ...
  temp1 = double(dsigma.transpose() * sigma)
      * temp0_reci; //(dsigma.transpose() * sigma) / temp0_reci; // dsigma^T * sigma /  ||dsigma||_2
  ...
  // d1_ego - d4_ego
  signed_dists(0) = log_sum_exp(-alpha, surround_signed_dists1, surround_exp_sum1) - temp1 - veh_param_.d_cr()
      - veh_param_.length() / 2.0;
  signed_dists(1) = log_sum_exp(-alpha, surround_signed_dists2, surround_exp_sum2) + temp1 + veh_param_.d_cr()
      - veh_param_.length() / 2.0;
  ...
  // d1_sur = d4_sur
  signed_dists(4) =
      log_sum_exp(-alpha, ego_signed_dists1, ego_exp_sum1) - temp_sur1 - veh_param_.d_cr() - veh_param_.length() / 2.0;
  signed_dists(5) =
      log_sum_exp(-alpha, ego_signed_dists2, ego_exp_sum2) + temp_sur1 + veh_param_.d_cr() - veh_param_.length() / 2.0;
...
double PolyTrajOptimizer::dynamicObsGradCostP(
  ...
  for (size_t sur_id = 0; sur_id < surround_trajs_->size(); sur_id++) {
    ...
    ego_normal_vectors_vec.clear(); // This vector is used to store the normal vectors of each hyperplane of the ego car
    std::vector<Eigen::VectorXd> vec_d_Uo_e_vec;
    vec_d_Uo_e_vec.clear();
    std::vector<Eigen::Matrix2d> F_delta_le_vec;
    F_delta_le_vec.clear(); // This vector stores the F(delta_le) which is later used in the gradient calculation
    std::vector<Eigen::Matrix2d> F_le_vec;
    F_le_vec.clear(); // This vector stores the F(le) which is later used in the gradient calcualtion
    for (int e = 0; e < number_of_hyperplanes_of_ego_car; e++) {
      ...
      Eigen::Matrix2d
          F_delta_le = singul_ * temp_l_Bl.transpose() * temp0_reci - dsigma * (ego_R * delta_le).transpose() * temp3;
      ...
      Eigen::VectorXd d_Uo_e_vec
          (number_of_hyperplanes_of_surround_car); // vector stores the d_Uo_e in the paper which is later put into the lse function
    ...
    surround_normal_vectors_vec.clear(); // This vector is used to store the normal vectors of each hyperplane of the surround car
    ...
    for (int o = 0; o < number_of_hyperplanes_of_surround_car; o++) {
      ...
      Eigen::VectorXd d_Ee_o_vec
          (number_of_hyperplanes_of_ego_car); // vector stores the d_Ee_o in the paper which is later put into the lse function
    ...
    for (int o = 0; o < number_of_hyperplanes_of_surround_car; o++) {
      ...
      partial_G_over_partial_sigma -=
          d_test(o + number_of_hyperplanes_of_ego_car) / exp_sum_d * partial_d_E_o_tilde_over_partial_sigma;
    ...
    for (int o = 0; o < number_of_hyperplanes_of_surround_car; o++) {
      ...
      partial_G_over_partial_dsigma -=
          d_test(o + number_of_hyperplanes_of_ego_car) / exp_sum_d * partial_d_E_o_over_partial_dsigma;
    }
    /*--------------------------------------------------------------------------------------*/
    ...
    for (int o = 0; o < number_of_hyperplanes_of_surround_car; o++) {
      ...
      for (int e = 0; e < number_of_hyperplanes_of_ego_car; e++) {
        double d_Ee_o = vec_d_Ee_o_vec[o](e);
        Eigen::Vector2d le = vec_le[e];
        double partial_d_Ee_o_over_partial_t_hat =
            ((ego_R * le).transpose() * B_h * R_u_t_hat_dot * delta_lo / delta_lo.norm())(0, 0);

        partial_d_E_o_over_partial_t_hat += d_Ee_o / d_Ee_o_exp_sum * partial_d_Ee_o_over_partial_t_hat;
      }

      partial_G_over_partial_t_hat -=
          d_test(o + number_of_hyperplanes_of_ego_car) / exp_sum_d * partial_d_E_o_over_partial_t_hat;
    }
    /*--------------------------------------------------------------------------------------*/
    ...
    jerkOpt_container[trajid].get_gdC().block<6, 2>(pieceid * 6, 0) +=
        omg * step * wei_surround_ * violaDynamicObsPenaD * gradViolaPc; // j gradient to c
...
void PolyTrajOptimizer::setSurroundTrajs(plan_utils::SurroundTrajData *surround_trajs_ptr) {
