...
void KinoAstar::getKinoNode(plan_utils::KinoTrajData &flat_trajs) {
  ...
  int lastS =
      (SampleTraj[1] - SampleTraj[0]).head(2).dot(Eigen::Vector2d(cos(SampleTraj[0][2]), sin(SampleTraj[0][2]))) >= 0
      ? 1 : -1;
  ...
  if (shot_timeList.size() >= 2) {
    ...
    if (shot_SList[shot_timeList.size() - 1] > 0)
      shot_timeList[shot_timeList.size() - 1] =
          evaluateDuration(shot_lengthList.back(), max_forward_vel, max_forward_acc, non_siguav, endvel);
    else
      shot_timeList[shot_timeList.size() - 1] =
          evaluateDuration(shot_lengthList.back(), max_backward_vel, max_backward_acc, non_siguav, endvel);
  } else {
    if (shot_SList[0] > 0)
  ...
  for (int i = 0; i < shot_lengthList.size(); i++) {
    ...
    traj_pts.push_back(Eigen::Vector3d(localTraj.back()[0],
                                       localTraj.back()[1],
                                       shot_timeList[i] - (samplet - sampletime)));
    thetas.push_back(localTraj.back()[2]);
    plan_utils::FlatTrajData flat_traj;
    Eigen::MatrixXd startS;
    Eigen::MatrixXd endS;
    getFlatState(Eigen::Vector4d(localTraj.front()[0], localTraj.front()[1], localTraj.front()[2], initv),
                 Initctrlinput,
                 startS,
                 sig);
    getFlatState(Eigen::Vector4d(localTraj.back()[0], localTraj.back()[1], localTraj.back()[2], finv),
                 Finctrlinput,
                 endS,
                 sig);
...
double KinoAstar::evaluateLength(double curt,
                                 double locallength,
                                 double localtime,
                                 double max_vel,
                                 double max_acc,
  ...
  if (locallength >= critical_len) {
    ...
    if (curt <= t1) {
      ...
      return startV * t1 + 0.5 * max_acc * pow(t1, 2) + (t2 - t1) * max_vel + max_vel * (curt - t2)
          - 0.5 * max_acc * pow(curt - t2, 2);
    }
  } else {
